...
class Form(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        w_crop_rotations = QCheckBox('Crop Rotations?')
        helpText = 'Selects crop rotations'
        w_crop_rotations.setToolTip(helpText)
        # w_crop_rotations.setEnabled(False)
        w_crop_rotations.stateChanged.connect(self.runCropRotationsChecked)
        grid.addWidget(w_crop_rotations, 18, 4)
        self.w_crop_rotations = w_crop_rotations
        ...
        w_crops = QPushButton('Crops')
        helpText = 'Runs ORATOR crops model'
        w_crops.setToolTip(helpText)
        #w_crops.setEnabled(False)
        w_crops.clicked.connect(self.runCropsClicked)
        grid.addWidget(w_crops, 19, 4)
        self.w_crops = w_crops
        ...
        w_npp_label = QLabel(self)
        helpText = 'Select method used to calculate crop NPP'
        w_npp_label.setText("NPP calculation method: ")
        w_npp_label.setToolTip(helpText)
        grid.addWidget(w_npp_label, 20, 0)

        w_npp_calc = QComboBox(self)
        helpText = 'Select method used to calculate crop NPP'
        w_npp_calc.addItem('MIAMI Model (Leith 1972)')
        #w_npp_calc.addItem('Growing degree days and water stress (Zaks et al. 2007)')
        #w_npp_calc.addItem('DO NOT USE Nutrient limitation (Reid 2002)')
        w_npp_calc.activated[str].connect(self.runNPPMethod)
        grid.addWidget(w_npp_calc, 20, 1)
        self.w_npp_calc = w_npp_calc
    ...
    def runCropsClicked(self):

        test_crop_algorithms(self)

    def runCropRotationsChecked(self, checked):

        if Qt.Checked == checked:
            crop_rotations = True
            print('Crop rotations used')

        else:
            crop_rotations = False
            print('Single crops used')

    def runNPPMethod(self, text):
        self.w_npp_calc.setCurrentText(text)
        print(f'Current choice is: {text}')
